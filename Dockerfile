...
RUN \
    ...
    tar zxf /tmp/traefik-auth.tar.gz --strip-components=1 -C /usr/local/bin/plugins-local/src/oidc; \
    ## real-ip
    wget -qO "/tmp/real-ip.tar.gz" https://github.com/Paxxs/traefik-get-real-ip/archive/refs/tags/v1.0.4.tar.gz; \
    mkdir -p /usr/local/bin/plugins-local/real-ip; \
    tar zxf /tmp/traefik-real-ip.tar.gz --strip-components=1 -C /usr/local/bin/plugins-local/real-ip; \
    ## clean.
    rm -rf /tmp/*
